...
WITH src AS (
    SELECT *
    FROM (SELECT
        split_part("название", ':', 2)::INTEGER AS empl_id,
...
SELECT
    ...
    src.date::DATE
FROM src
INNER JOIN dds_ksusha.levels AS l
    ON src.level_id = l.id
INNER JOIN dds_ksusha.skills AS s
    ON src.skill_id = s.id
INNER JOIN dds_ksusha.employees AS e
    ON src.empl_id = e.id;
...
WITH src AS (
    ...
    FROM (
        SELECT
            "дата" AS date,
            id,
            split_part("название", ':', 2) AS empl_id,
